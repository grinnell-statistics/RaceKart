...
/*
* Class: Arcade Kart
* Purpose: Controls how the kart moves
* Modified by: Priyanka
*               - takes in value from 'Test' script for Sandbox
*               - puts different values to create "slippery effect"
*               - and "desert effect" for Desert() and Mountain tracks
* Modified by: Ryuta
*               - to record topSpeedAchieved / timeToTopSpeed
*/
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public static bool SandboxChosen;
        // Coasting Drag : 5-15
        private float ConstrictedCoastingDrag;
        // Coasting Drag : 5-15
        private float ConstrictedSteer;
        // Coasting Drag : 0.5-0.9
        private float ConstrictedGrip;

        // Ryuta Code
        public float topSpeedAchieved;
        ...
        public struct Stats
        {
            ...
            {
                // if the user has come from the sandbox scene switch
                if (prevSceneNum == 18 )
                {
                    SandboxChosen = true;

                    if (SceneManager.GetActiveScene().name == "MountainTrack"){

                        float ConstrictedCoastingDrag = 0f;
                        float ConstrictedTopSpeedFloat = Test.TopSpeedFloat + 5f;
                        float ConstrictedAccelerationFloat = Test.AccelerationFloat - 2f;
                        float ConstrictedAccelerationCurveFloat = Test.AccelerationCurveFloat - 0.2f;
                        float ConstrictedGripFloat = Test.GripFloat - 0.25f;

                        if (Test.CoastingDragFloat > 5f)
                        {
                            ConstrictedCoastingDrag = 5f;
                        }
                    
                        float ConstrictedSteer = Test.SteerFloat + 3f;
                        
                        return new Stats
                        {
                            Acceleration        = ConstrictedAccelerationFloat + b.Acceleration,
                            AccelerationCurve   = ConstrictedAccelerationCurveFloat + b.AccelerationCurve,
                            Braking             = a.Braking + b.Braking,
                            CoastingDrag        = Test.CoastingDragFloat - ConstrictedCoastingDrag,
                            AddedGravity        = Test.GravityFloat + b.AddedGravity,
                            Grip                = ConstrictedGripFloat + b.Grip,
                            ReverseAcceleration = a.ReverseAcceleration + b.ReverseAcceleration,
                            ReverseSpeed        = a.ReverseSpeed + b.ReverseSpeed,
                            TopSpeed            = ConstrictedTopSpeedFloat + b.TopSpeed,
                            Steer               = ConstrictedSteer + b.Steer,
                            Suspension          = Test.SuspensionFloat + b.Suspension
                        };
                    }
                    else if (SceneManager.GetActiveScene().name == "WindingTrack")
                    {
                        
                        float ConstrictedCoastingDrag = Test.CoastingDragFloat + 10f;
                        float ConstrictedSteer = Test.SteerFloat - 1.5f;
                        float ConstrictedGrip = Test.GripFloat - 0.2f;
                        float ConstrictedSuspension = Test.SuspensionFloat + 0.3f;
                        float ConstrictedAccelerationCurve = Test.AccelerationCurveFloat - 0.1f;
                        float ConstrictedAcceleration = Test.AccelerationFloat - 1f;

                        
                        return new Stats
                            {
                            Acceleration        = ConstrictedAcceleration + b.Acceleration,
                            AccelerationCurve   = ConstrictedAccelerationCurve + b.AccelerationCurve,
                            Braking             = a.Braking + b.Braking,
                            CoastingDrag        = ConstrictedCoastingDrag + b.CoastingDrag,
                            AddedGravity        = Test.GravityFloat + b.AddedGravity,
                            Grip                = ConstrictedGrip + b.Grip,
                            ReverseAcceleration = a.ReverseAcceleration + b.ReverseAcceleration,
                            ReverseSpeed        = a.ReverseSpeed + b.ReverseSpeed,
                            TopSpeed            = Test.TopSpeedFloat + b.TopSpeed,
                            Steer               = ConstrictedSteer + b.Steer,
                            Suspension          = ConstrictedSuspension + b.Suspension
                        };
                    }
                    else {
                        return new Stats
                        {
                            Acceleration        = Test.AccelerationFloat + b.Acceleration,
                            AccelerationCurve   = Test.AccelerationCurveFloat + b.AccelerationCurve,
                            Braking             = a.Braking + b.Braking,
                            CoastingDrag        = Test.CoastingDragFloat + b.CoastingDrag,
                            AddedGravity        = Test.GravityFloat + b.AddedGravity,
                            Grip                = Test.GripFloat + b.Grip,
                            ReverseAcceleration = a.ReverseAcceleration + b.ReverseAcceleration,
                            ReverseSpeed        = a.ReverseSpeed + b.ReverseSpeed,
                            TopSpeed            = Test.TopSpeedFloat + b.TopSpeed,
                            Steer               = Test.SteerFloat + b.Steer,
                            Suspension          = Test.SuspensionFloat + b.Suspension
                        };
                    }
                ...
                //if the user is in the MountainTrack then make the track slippery
                // i.e. the car keeps gliding even if no acceleration is pressed
                // and the steering is a bit hard

                else if ((prevSceneNum != 18) && (SceneManager.GetActiveScene().name == "MountainTrack")) {
                    
                    SandboxChosen = false;
                    float ConstrictedCoastingDrag = 0f; 

                    //reduces 5f from the car model's coasting drag only if it is already set at above 5
                    // so that it doesn't become a negative number
                    if (a.CoastingDrag > 5f)
                    {
                        ConstrictedCoastingDrag = 5f;
                    }
                    ...
                    return new Stats
                    {
                        Acceleration        = a.Acceleration - 2f, // altered value
                        AccelerationCurve   = a.AccelerationCurve - 0.2f, // altered value
                        Braking             = a.Braking + b.Braking,
                        CoastingDrag        = a.CoastingDrag - ConstrictedCoastingDrag, //altered value
                        AddedGravity        = a.AddedGravity + b.AddedGravity,
                        Grip                = a.Grip - 0.25f, // altered value
                        ReverseAcceleration = a.ReverseAcceleration + b.ReverseAcceleration,
                        ReverseSpeed        = a.ReverseSpeed + b.ReverseSpeed,
                        TopSpeed            = a.TopSpeed + 5f, // altered value
                        Steer               = a.Steer + 3f, //altered value
                        Suspension          = a.Suspension + b.Suspension
                    };
                }

                // if in the desert scene, the car stops immediately due to sand friction
                // the amount of side-to-side friction for the wheels is high 
                // slower turns due to friction

                else if ((prevSceneNum != 18) && (SceneManager.GetActiveScene().name == "WindingTrack")) {
                    
                    SandboxChosen = false;

                    float ConstrictedSteer = 0f;
                    float ConstrictedGrip = 0f;

                     //reduces certain float val from the car model's steer/grip only if it is already set 
                    // at above x value so that it doesn't become a negative number
                    if (a.Steer> 7f)
                    {
                        ConstrictedSteer = 3f;
                    }
                    ...
                    if (a.Grip < 0.91f)
                    {
                        ConstrictedGrip = 0.1f;
                    }


                    return new Stats
                    {
                        Acceleration        = a.Acceleration - 1f, // altered value
                        AccelerationCurve   = a.AccelerationCurve - 0.1f, // altered value
                        Braking             = a.Braking + b.Braking,
                        CoastingDrag        = a.CoastingDrag + 10f, //altered value
                        AddedGravity        = a.AddedGravity + b.AddedGravity,
                        Grip                = a.Grip - 0.2f, //altered value
                        ReverseAcceleration = a.ReverseAcceleration + b.ReverseAcceleration,
                        ReverseSpeed        = a.ReverseSpeed + b.ReverseSpeed,
                        TopSpeed            = a.TopSpeed + b.TopSpeed,
                        Steer               = a.Steer - 1.5f, //altered value
                        Suspension          = a.Suspension + 0.3f // altered value
                    };
                }

                // if the user has come from another scene go normally
                else {
        ...
        public ArcadeKart.Stats finalStats;
        ...
        void MoveVehicle(float accelInput, float turnInput)
        {
            ...
            // Ryuta: Record topSpeedAchieved
            if (topSpeedAchieved <= currentSpeed)
            {
                // records topspeed
                topSpeedAchieved = currentSpeed;
                // collects time taken to reach topspeed
                timeToTopSpeed = TimeManager.timeTaken;
            }
